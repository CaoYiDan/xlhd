...
//  MTUserd
//
//  Created by 伯爵 on 2020/1/4.
//  Copyright © 2020 MTShopd. All rights reserved.
//

#import "YkczView.h"
#import "YkczInfoView.h"

@implementation YkczView
...
- (void)getDataWithInfo:(NSArray *)datas{
    ...
    for (NSInteger i = 0; i < datas.count; i++) {
        ...
        YkczInfoView *view = [YkczInfoView loadFromNIB];
        ...
        view.dayLab.text = [NSString stringWithFormat:@"%@天 不限次",model.days];
...
- (void)chooseBtnClick:(UIButton *)btn{
    ...
    self.goodId = fModel.goods_id;
    for (YkczInfoView *vi in self.viewArray) {
        if (vi.tag == btn.tag) {
