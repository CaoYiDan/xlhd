...
//        [_searchBar setImage:[UIImage imageNamed:@"1"] forSearchBarIcon:UISearchBarIconSearch state:UIControlStateNormal];
    ...
    self.scrollView.contentSize = CGSizeMake(0,self.bgView.height+10);
//    禁止垂直方向的滑动：scrollView.contentSize(width,0);
//    禁止水平方向的滑动：scrollView.contentSize(0,height);
    ...
    self.firstView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight-70-64)];
    self.firstView.backgroundColor = RGB(236, 237, 238);
    ...
    _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight-70-64-49-20)style:UITableViewStylePlain];
...
-(void)addLocationBtn{
    ...
    [baseView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.offset(-49-34-60);
        make.right.offset(-12);
    }];
    
    UIImageView *img = [[UIImageView alloc]init];
    img.image = [UIImage imageNamed:@"定位-1"];
...
- (void)requestData:(NSString *)searchStr{
    ...
    if (searchStr) {
        [param setValue:searchStr forKey:@"name"];
    }
...
- (MAAnnotationView *)mapView:(MAMapView *)mapView viewForAnnotation:(id<MAAnnotation>)annotation
{
    ...
    {
        static NSString *reuseIndetifier = @"lsannotationReuseIndetifier";
...
- (IBAction)rightBtnClick:(UIButton *)sender {
    GzListViewController *gzListVC = [[GzListViewController alloc]init];
    [self.navigationController pushViewController:gzListVC animated:YES];
}
